...
const loginUser = () => {
  return authStore.loginUser({ email: email.value, password: password.value })
}
  ...
  <main>
    <form method="post" class="form" novalidate @submit.prevent="tryLogin">
      ...
      <div class="form-title">
        ...
        <h1>Log in om verder te gaan <span> Fijn je weer te zien!</span></h1>
      </div>
      <FormSummary
        :success="showSuccess"
        :errors="errors"
      ...
      <RouterLink :to="{ name: 'send-forgot-password' }"
      ...
      <!--
        ...
        Log in met Microsoft Office</FButton
        >
        -->
      <p class="no-account">
        Nog geen account?
        <RouterLink :to="{ name: 'register' }">Registreer</RouterLink>
      </p>
    </form>
  </main>
</template>
